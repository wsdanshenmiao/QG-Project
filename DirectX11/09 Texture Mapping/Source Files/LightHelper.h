...
struct DirectionalLight
{
    ...
    DirectX::XMFLOAT3 direction;    //方向光照射方向
...
struct PointLight
{
    ...
    DirectX::XMFLOAT3 position; //点光源位置
    float range;

    // 打包成4D向量: (A0, A1, A2, pad)
    DirectX::XMFLOAT3 att;  //衰弱系数
    float pad; // 最后用一个浮点数填充使得该结构体大小满足16的倍数，便于我们以后在HLSL设置数组
};
...
struct SpotLight
{
    ...
    // 打包成4D向量: (position, range)
    DirectX::XMFLOAT3 position; //聚光灯位置
    float range;

    // 打包成4D向量: (direction, spot)
    DirectX::XMFLOAT3 direction;    //聚光灯照射方向
    float spot;

    // 打包成4D向量: (att, pad)
    DirectX::XMFLOAT3 att;  //聚光灯衰弱系数
    float pad; // 最后用一个浮点数填充使得该结构体大小满足16的倍数，便于我们以后在HLSL设置数组
};
