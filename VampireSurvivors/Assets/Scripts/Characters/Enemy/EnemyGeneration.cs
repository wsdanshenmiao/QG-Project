...
{
    public Dictionary<string, EnemyPool> EnemyPools = new Dictionary<string, EnemyPool>();
    ...
    private void Start()
    {
        ...
        foreach(EnemyStats enemy in m_EnemyPrefab){
            GameObject poolHolder = new GameObject($"Pool:{enemy.name}");
            poolHolder.transform.SetParent(transform);

            EnemyPool pool = poolHolder.AddComponent<EnemyPool>();
            pool.SetPrefab(enemy);

            EnemyPools.Add(enemy.EnemyID, pool);
        }
    }
    ...
    private void GenerateEnemy()
    {
        // 随机选择怪物池
        EnemyPool enemyPool;
        if(!EnemyPools.TryGetValue(m_EnemyPrefab[Random.Range(0,m_EnemyPrefab.Length)].name, out enemyPool)){
            return;
        }
        // 随机选择一个区块
        ...
        for (int i = 0; i < m_PreWaveEnemyNum; ++i) {
            ...
            EnemyStats enemy = enemyPool.Get();
            enemy.transform.position = new Vector3(x - mapData.MapWidth * 0.5f, y - mapData.MapHeight * 0.5f, 0);
        }
    }
    ...
    private IEnumerator CheckEnemyCount()
    {
        ...
        foreach(var pool in EnemyPools){
            totalActiveCout += pool.Value.CountActive;
        }
